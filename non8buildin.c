...
/**
* nonbuiltin_hondler - Handle non-builtin commands
						...
						int count, list_paths *current, char *argv[])
{
	...
	if (access(command_array[0], X_OK) == 0)
		execute_command(command_array[0], command_array, env, status);
	else
	{
		/*if the user just entered the command without the whole path*/
		new_path = check_acces(command_array[0], current);
		if (new_path)
		{
			execute_command(new_path, command_array, env, status);
			free(new_path);
		}
		else
		{
			print_err(argv[0], count, command_array[0], NOT_FOUND);/*----lsa----*/
			*status = NOT_FOUND;
		}
	}
}
